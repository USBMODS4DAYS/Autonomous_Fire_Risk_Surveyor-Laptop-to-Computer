...
class HotspotDetector(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"HotspotDetector listening on [{image_topic}]")
    ...
    def image_callback(self, msg: Image):
        ...
        if top_conf >= self.conf_thresh:
            m = Marker()
            # NEW: use the same frame as the incoming image → works for Husky or drone
            ...
            # Just put the text 1m in front of the camera frame
