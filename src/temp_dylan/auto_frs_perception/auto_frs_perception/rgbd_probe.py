...
class Probe(Node):
    def __init__(self):
        ...
        # NEW: configurable depth topic
        self.declare_parameter('depth_topic', '/camera/depth/points')
        depth_topic = self.get_parameter('depth_topic').value

        self.sub = self.create_subscription(
            PointCloud2, depth_topic, self.cb, 10)

        self.get_logger().info(f"rgbd_probe listening to depth on [{depth_topic}]")
    ...
    def cb(self, msg: PointCloud2):
        ...
        self.get_logger().info(
            f'Center @ {msg.header.frame_id}: ({x:.2f}, {y:.2f}, {z:.2f}) m'
        )
        ...
        for target in ('map', 'odom'):
            try:
                ...
                self.get_logger().info(
                    f'Center @ {target}: ({out.point.x:.2f}, {out.point.y:.2f}, {out.point.z:.2f}) m'
                )
                break
            except Exception:
                pass
